...
beforeEach(async () => {
  await page.goto('http://localhost:20000/economicfreedom_stats');
...
test('load and list economic freedom stats', async () => {
  test.setTimeout(60000);
  await page.goto('http://localhost:20000/api/v2/economicfreedom_stats/loadInitialData');
  await page.goto('http://localhost:20000/economicfreedom_stats');
  await page.waitForLoadState('load');
  await page.waitForTimeout(1000);
...
test('edit resource', async () => {
  test.setTimeout(60000);
  await page.goto('http://localhost:20000/api/v2/economicfreedom_stats/loadInitialData');
  await page.goto('http://localhost:20000/economicfreedom_stats/2017/Denmark');
  await page.waitForLoadState('load');
  await page.waitForTimeout(1000);
  await page.fill('#overallScore', '2');
  await page.click('button:text("Actualizar")');
  await page.waitForTimeout(1000);
  let successMessage = await page.$('text=El dato con pais Denmark y año 2017 se ha actualizado correctamente');
  expect(successMessage).not.toBeNull();
});
